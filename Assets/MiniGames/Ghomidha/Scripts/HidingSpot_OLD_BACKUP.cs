...
namespace Ghomidha
{
    ...
    /// </summary>
    public class HidingSpot_Backup : MonoBehaviour
    {
        // ── Inspector ────────────────────────────────────────────────────
        ...
        [SerializeField] private string unhideText = "[ EXIT ]";
        ...
        private static HidingSpot_Backup s_activeSpot = null;

        // ── Runtime references ───────────────────────────────────────────
        private Canvas      hideCanvas,   unhideCanvas;
        private CanvasGroup hideGroup,    unhideGroup;
        private float       hideAlpha,    unhideAlpha;

        private XROrigin  xrOrigin;             // XRI component — used for MoveCameraToWorldLocation
        private Transform xrOriginTransform;    // convenience shortcut to its transform
        ...
        {
            if (IsHiding)                              return;
            if (xrOrigin == null || hidePosition == null) return;

            // ── Save pre-hide state for UNHIDE restore ──
            preHideOriginPosition = xrOriginTransform.position;
            preHideOriginRotation = xrOriginTransform.rotation;

            // ── Teleport: use XRI's built-in API so the CAMERA lands at hidePosition X/Z ──
            // MoveCameraToWorldLocation correctly handles the Player parent and all internal
            // XR Rig offsets — our manual camOffset math did NOT account for the parent.
            float camY = Camera.main != null ? Camera.main.transform.position.y : xrOriginTransform.position.y;
            xrOrigin.MoveCameraToWorldLocation(new Vector3(hidePosition.position.x, camY, hidePosition.position.z));
            xrOriginTransform.rotation = hidePosition.rotation;

            // Lock at wherever the XR Origin landed after MoveCameraToWorldLocation
            lockedXZ       = new Vector3(xrOriginTransform.position.x, 0f, xrOriginTransform.position.z);
            positionLocked = true;
